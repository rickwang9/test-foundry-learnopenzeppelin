...
abstract contract Governor is Context, ERC165, EIP712, IGovernor, IERC721Receiver, IERC1155Receiver {
    ...
    constructor(string memory name_) EIP712(name_, version()) {
        SLOG('constructor', name_);
        _name = name_;
    }

    /**
        接收将由州长处理的 ETH 的功能（如果执行者是第三方合约则禁用）
    ...
    /**
     * 查询提案状态（Pending Active Succeeded Defeated）
     */
    function state(uint256 proposalId) public view virtual override returns (ProposalState) {
        SLOG('Governor.state',proposalId);
        ...
        SLOG('_quorumReached(proposalId)',_quorumReached(proposalId));
        SLOG('_voteSucceeded(proposalId)',_voteSucceeded(proposalId));
    ...
    function propose(
        ...
        console2.log('proposer------------------------------------------------------------',_msgSender());
        SLOG('proposer------------------------------------------------------------');
        address proposer = _msgSender();
        SLOG('proposer', proposer);
        require(_isValidDescriptionForProposer(proposer, description), "Governor: proposer restricted");

        uint256 currentTimepoint = clock();
        console2.log('getVotes(proposer, currentTimepoint - 1)------------------------------------------------------------',getVotes(proposer, currentTimepoint - 1));
        require(
    ...
    function _isValidDescriptionForProposer(
        ...
        // Length is too short to contain a valid proposer suffix 32+20"#proposer=“？
        if (len < 52) {
            return true;
        }
